...
## Purpose
Map 19 foundational computer science disciplines relevant to the research domain, identifying their theoretical primitives, core guarantees, and boundary limitations.
...
*   **Boundary Limitation:** Captures invocation mechanics but does not internally model the causal justification or derivation trajectory that prompted the invocation. (See RQ-0: $\text{Invoke}(e) \iff \text{token} \in \text{Context}$)
...
*   **Boundary Limitation:** Static type-level guarantees constrain code as written. They do not extend to runtime-derived execution parameters generated by a non-deterministic evaluation engine operating over dynamic contextual inputs.
...
*   **Boundary Limitation:** Accountability systems bind actions to identities, not to the causal decision derivation that generated the action parameters.
...
*   **Boundary Limitation:** Verification applies to statically defined system models. Non-deterministic runtime derivation processes introduce state spaces that may exceed tractable verification boundaries.
...
*   **Boundary Limitation:** Hardware attestation proves that code-as-compiled executed within an enclave, but does not structurally bind the dynamic runtime evaluation trajectory to the delegation lineage. If the attested code contains a vulnerability in decision derivation, the attestation remains valid despite the semantic failure.
...
*   **Boundary Limitation:** Static type-level guarantees constrain code as written and compiled. They do not extend to dynamically loaded runtime modules or execution trajectories derived at runtime that were not present at compile time.
...
*   **Boundary Limitation:** All established program logics require the program statement $c$ to be known and fixed at proof time. When $c$ is itself the output of a runtime derivation process whose evaluation relation is not fixed before execution, the standard triple $\{P\} \, c \, \{Q\}$ becomes undefined until $c$ is known — at which point the execution has already occurred.
...
*   **Boundary Limitation:** Static analysis operates over a fixed program text or control-flow graph. When the control-flow graph itself is derived at runtime (e.g., a query planner constructing its own execution plan), static analysis cannot track invariants through that derivation boundary without requiring the derivation be fully pre-enumerable.
...
*   **Boundary Limitation (Post-Facto Proof System Posture):** These frameworks generate certificates ($\pi$) for computations or proofs internal to their formal domain ($\text{CheckCert}(e, \pi) \equiv \text{Valid}$). Our survey has not identified a standard formulation that directly treats delegated authority ($\Lambda$) as a first-class semantic assumption carried through runtime derivations under an adversarial execution model.

### 19. Secure Compilation
*   **Core Guarantee:** Formal preservation of security properties (e.g., Robust Safety Preservation, Robust Hyperproperty Preservation, and Fully Abstract Compilation) when translating programs from a source language to a lower-level or adversarial target environment.
*   **Boundary Limitation:** Focuses on translating fixed source programs to lower-level binaries safely. It does not model execution environments whose explicit runtime task is to derive and execute brand new, arbitrary query plans or workflows based on incoming delegation objects ($\Lambda$).
