...
class ExecutionGraph:
    ...
    def add_node(self, node_id: str, node_type: str, payload: Dict[str, Any], parent_id: Optional[str] = None) -> None:
        node = GraphNode(node_id=node_id, node_type=node_type, payload=payload, parent_id=parent_id)
        self.nodes[node_id] = node
        if parent_id and parent_id in self.nodes:
            self.nodes[parent_id].children.append(node_id)

    def render_tree(self, current_id: Optional[str] = None, depth: int = 0) -> str:
