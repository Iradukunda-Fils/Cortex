...
from tools.kernel.schema.message import Intent, Plan, Command, DriverTelemetryEvent, VerificationResultEvent
...
class TestStage4AVerticalSlice(unittest.TestCase):
    ...
    def test_end_to_end_autonomous_flow(self):
        ...
        # 1. Dispatch Intent
        intent = Intent(session_id="sess_4a", goal="Execute Maintenance Routine")
        ...
        # 3. Executor produces Commands
        commands = executor.handle_plan(plan)
        self.assertEqual(len(commands), 2)

        # 4. Driver executes Commands and produces Telemetry
        telemetries = [driver.handle_command(cmd) for cmd in commands]
        self.assertEqual(len(telemetries), 2)

        # 5. Verifier evaluates Telemetry and produces Verification Results
        ...
        # 6. Verify Execution Graph DAG & Lineage Reconstruction
        ...
        # Check total nodes in graph
        self.assertEqual(len(graph.nodes), 8) # 1 Intent + 1 Plan + 2 Commands + 2 Telemetries + 2 Verifications
