...
class TestOfflineNamespaceHygiene(unittest.TestCase):
    ...
    def test_cortex_security_references_classification(self):
        ...
        for ref in found_references:
            ...
            is_json_schema_id = ('"$id"' in ref["content"] or '"$id":' in ref["content"]) and ref["file"].endswith(".schema.json")
        ...
        for file in os.listdir(workflows_dir):
            if file.endswith((".yml", ".yaml")):
                ...
                with open(file_path, "r", encoding="utf-8") as f:
                    content = f.read()
                self.assertNotIn("cortex.security", content, f"CI workflow {file} contains reference to cortex.security!")
