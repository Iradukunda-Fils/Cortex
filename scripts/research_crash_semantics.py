...
def main() -> None:
    ...
    for key, sc in results["scenarios"].items():
        print(f"[{sc['title']}]")
        final_st = (
            sc.get("final_state") or f"WF1: {sc.get('workflow_1_final_state')}, WF2: {sc.get('workflow_2_final_state')}"
        )
        ...
        if "stage_c_executed" in sc:
            print(
                f"  - Prior Events Preserved: {sc['prior_events_preserved_in_store']} | Stage C Executed: {sc['stage_c_executed']}"
            )
