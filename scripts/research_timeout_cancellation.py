...
def main() -> None:
    ...
    for key, sc in results["scenarios"].items():
        ...
        if "downstream_stage_3_halted" in sc:
            print(
                f"  - Stage 1 Preserved: {sc['stage_1_events_preserved']} | Downstream Halted: {sc['downstream_stage_3_halted']}"
            )
        ...
        if "subsequent_workflow_healthy" in sc:
            print(
                f"  - WF1: {sc['workflow_1_final_state']} | WF2: {sc['workflow_2_final_state']} | Healthy: {sc['subsequent_workflow_healthy']}"
            )
