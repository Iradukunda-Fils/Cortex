...
def main() -> None:
    ...
    print("=================================================================")
    print(
        f"Environment: Python {results['environment']['python_version']} on {results['environment']['os']} ({results['environment']['arch']})"
    )
    print("-----------------------------------------------------------------")
    ...
    print(f"\n[{exp_b['title']}]")
    print(
        f"  - B1 (Pre-Execution): Mutations={exp_b['b1_pre_execution']['side_effect_mutations']} | Ambiguity={exp_b['b1_pre_execution']['ambiguity']}"
    )
    print(
        f"  - B2 (Mid-Execution): Mutations={exp_b['b2_mid_execution']['side_effect_mutations']} | Ambiguity={exp_b['b2_mid_execution']['ambiguity']}"
    )
    print(
        f"  - B3 (Post-Execution): Mutations={exp_b['b3_post_execution']['side_effect_mutations']} | Ambiguity={exp_b['b3_post_execution']['ambiguity']}"
    )

    exp_c = results["experiments"]["experiment_c"]
    print(f"\n[{exp_c['title']}]")
    print(
        f"  - Replay Without Idempotency Key: {exp_c['replay_without_idempotency_mutations']} mutations (DUPLICATED!)"
    )
