...
def main() -> None:
    ...
    print(
        f"  - P50: {wa['p50_ms']:.4f} ms | P95: {wa['p95_ms']:.4f} ms | P99: {wa['p99_ms']:.4f} ms | Mean: {wa['mean_ms']:.4f} ms"
    )
    print("Workload B (Multi-Stage 3-Plugin Chained Events):")
    print(f"  - Events: {wb['event_count']} | State: {wb['state_transitions'][-1]}")
    print(
        f"  - P50: {wb['p50_ms']:.4f} ms | P95: {wb['p95_ms']:.4f} ms | P99: {wb['p99_ms']:.4f} ms | Mean: {wb['mean_ms']:.4f} ms"
    )
    print("Workload C (Capability Violation Failure):")
    print(
        f"  - Events: {wc['event_count']} | State: {wc['state_transitions'][-1]} | Violations: {wc['verification_violations']}"
    )
    print(
        f"  - P50: {wc['p50_ms']:.4f} ms | P95: {wc['p95_ms']:.4f} ms | P99: {wc['p99_ms']:.4f} ms | Mean: {wc['mean_ms']:.4f} ms"
    )
    print("=================================================================")
